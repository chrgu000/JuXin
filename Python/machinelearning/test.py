...
if sw1:
    ...
    Rall=[];Rall1=[];Rall2=[];dateAll=[];Matrix=[];Matrix=[]
    Lstocks=len(stocks)
    for i in range(Lstocks):
        ...
        for i2 in range(10,Li-5):
            ...
            and 1.025 <=Close[i2]/Close[i2-1]<1.055 and Low[i2]/Low[i2-1]>=1.01:
                ...
                Rall1.append(Close[i2+2]/Close[i2]-1)
                Rall2.append(Close[i2+2]/Close[i2+1]-1)
    ...
    tem={'Rall':Rall,'Rall1':Rall1,'Rall2':Rall2,'dateAll':dateAll,'Matrix':Matrix}
    pickle.dump(tem,Tem)
    Tem.close()
...
tem=pickle.load(Tem)
Tem.close()
Rall=tem['Rall'];Rall1=tem['Rall1'];Rall2=tem['Rall2'];dateAll=tem['dateAll'];Matrix=tem['Matrix']
...
if sw2:    
    ...
    hmm=joblib.load('D:\Trade\Python\machinelearning\modelTestSpringHMM')
    Re=np.array(Rall)
    flag=hmm.predict(X) 
    buy=((flag==0)+(flag==2)+(flag==4))
