    ...
    % %     end
        ...
        for ii=15:L-5
            if close(ii-1)<low(ii-1)+(high(ii-1)-low(ii-1))*0.25 &&close(ii)/close(ii-1)-1<0.095&&high(ii)>low(ii)...%&&close(ii)>low(ii)+(high(ii)-low(ii))*0.1
                    &&close(ii)/close(ii-1)-1>=0.025&&close(ii)/close(ii-1)-1<0.055  && low(ii)/low(ii-1)-1>=0.01%&&lowTest<-0.01 %                    
                iRall=iRall+1;
                ...
                Matrix(iRall,:)=[close(ii+2)/close(ii)-0.003,close(ii+3)/close(ii+1)-0.003,(close(ii)-low(ii))/(high(ii)-low(ii)),...
                    std([close(ii);open(ii);low(ii);high(ii)])/std([close(ii-1);open(ii-1);low(ii-1);high(ii-1)]),maN(ii)/ma10(ii),...
                    close(ii)/close(ii-4),high(ii)/high(ii-1),low(ii)/low(ii-1),close(ii)/close(ii-2),close(ii)/low(ii-1),close(ii)/high(ii-1),...
                    close(ii)/mean([low(ii-1),high(ii-1)]),close(ii+2)/open(ii+1)-0.003]-1;
