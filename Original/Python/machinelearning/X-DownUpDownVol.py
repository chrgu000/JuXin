...
#        Lstocks=10
    ...
    for stockI in range(Lstocks):
        ...
        for i in range(20,L):
            ...
            abs(max(highs[i-20:i-10])-highs[i-15])<0.00000001 and \
            lows[i]<min(lows[i-5:i]) and \
            min(highs[i-15:i+1]-lows[i-15:i+1])>0.0000001:
                P1=i-15 #index 0 is i-3*i2-5
                P2=i-10
                P3=i-5
                P4=i
                down1=P2-P1
                ...
                try:
                    ...
                    continue                            
                features.append([dates[i],(highs[P3]-lows[P2])/baseL,(highs[P3]-lows[P4])/baseL,\
                ...
                if fig>0 and np.random.rand(1)>0.95:
                    ...
                    ax=plt.subplot()
                    candleData=np.column_stack([range(21),opens[i-20:i+1],highs[i-20:i+1],lows[i-20:i+1],closes[i-20:i+1]])
                    mpf.candlestick_ohlc(ax,candleData,width=0.5,colorup='r',colordown='g')
                    plt.plot([5,10,15,20],[highs[P1],lows[P2],highs[P3],lows[P4]],color='k')
