...
for i in range(len(stocks)):
    ...
    for i2 in range(4,20):
        ...
        if pointZero-backDay-1<0:
            pointZero=backDay+1
        if abs(max(highs[Li-3*i2-2-backDay:Li-3*i2+1-backDay])-max(highs[pointZero-backDay-1:Li-2*i2+1-backDay]))<0.00000001 and \
        abs(min(lows[Li-3*i2-backDay-1:Li-i2-backDay])-min(lows[Li-2*i2-2-backDay:Li-2*i2+1-backDay]))<0.00000001 and \
        abs(max(highs[Li-2*i2-backDay-1:Li-backDay])-max(highs[Li-i2-2-backDay:Li-i2+1-backDay]))<0.00000001 and \
        ...
        highs[Li-backDay]<highs[Li-backDay-1]:
            ...
            if max([down1,up,down2])-min([down1,up,down2])<3:# and fig>0:
                if figHere:
                    plt.clf()
                periodBoll=i2*3
                upLine=[]
                ...
                for i3 in range(baseI+periodBoll,Li+1):#periodBoll-1,Li+1-baseI
                    tmpMean=closes[i3-periodBoll:i3+1].mean()
                    tmpStd=closes[i3-periodBoll:i3+1].std()
                    upLine.append(tmpMean+2*tmpStd)
                    middleLine.append(tmpMean)
                    downLine.append(tmpMean-2*tmpStd)                   
                ax=fig.add_axes([0.1,0.3,0.8,0.6])
                ax.plot(range(periodBoll-1,Li+1-baseI),[])
