...
close=np.array(dataTem.Data[0])[~tem]
high=np.array(dataTem.Data[1])[~tem][5:]
low=np.array(dataTem.Data[2])[~tem][5:]
vol=np.array(dataTem.Data[3])[~tem][5:]
money=np.array(dataTem.Data[3])[~tem][5:]
dateList=pd.to_datetime(np.array(dataTem.Times)[~tem][5:])
...
dataTem=w.wsd(objectTrade,'close,high,low,volume','2012-10-05','2017-9-21','PriceAdj=F')
