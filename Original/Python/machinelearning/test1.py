...
def read_cifar_files(filename_queue,distort_images=True):
    ...
    image_uint8image=tf.transpose(image_extracted,[1,2,0]) #i don;t know what is meaning here
...
def input_pipeline(batch_size,train_logical=True):
    if train_logical:
        files=[os.path.join(data_dir,extract_folder,'data_batch_{}.bin'.format(i)) \
               for i in range(1,6)]
    else:
...
def cifar_cnn_model(input_images,batch_size,train_logical=True):
    ...
    with tf.variable_scope('conv2') as scope:
        conv2_kernel=truncated_normal_var(name='conv_kernel2',shape=[5,5,64,64],\
                        dtype=tf.float32)
    ...
    norm2=tf.nn.lrn(pool2,depth_radius=5,bias=2.0,alpha=1e-3,\
    ...
    with tf.variable_scope('full2') as scope:
        ...
        with tf.variable_scope('full3') as scope:
            full_weight3=truncated_normal_var(name='full_mult3',\
                                shape=[192,num_targets],dtype=tf.float32)
