# -*- coding: utf-8 -*-
"""
Created on Fri Apr 27 08:24:49 2018
                ...
                if abs(min(lows[i-i2-5:i-i2+3])-lows[i-i2])<0.00000001:
                    ...
                    min(highs[i-3*i2//2-3:i+1]-lows[i-3*i2//2-3:i+1])>0.0000001: # avoid no trading day;
                        ...
                        if i+i2//2>L-1: #if this profit can't be caculated by future bars;
                            break
                        try:
                            baseE=sum(vols[i-i2-i2//2:i-i2]) 
                            if baseE<1:                         
                                continue
                        except:
                            continue                        
                        features.append([dates[i],down/up,sum(vols[highT:i])/baseE,sum(vols[i-i2:highT])/baseE])
                        stopL=lows[i]
...
if useXGB:
    ...
    if reTrain:
        XGB=xgb.train(param,data_train,num_boost_round=10000,evals=watch_list)
