# -*- coding: utf-8 -*-
"""
Created on Tue Sep 19 14:13:02 2017
...
x1=time.clock()
nameDB='Up2Down2' # should be set for create a new mode test; if future should be 'futuretest'
firstTime=0
shufflePoints=0
ReGetPoints=0
TradeScan=0
...
def ModelX(opens,highs,lows,closes):
    if lows[-4]<=min(lows[-6:]) and highs[-3]>highs[-4] and highs[-2]>highs[-1] and lows[-2]>lows[-1] and \
    highs[-4]>lows[-4] and highs[-3]>lows[-3]and highs[-2]>lows[-2]and highs[-1]>lows[-1] and closes[-1]/closes[-2]<1.095: #vols[i2-2:i2].min()>vols[[i2-3,i2]].max() and 
        R=1
    else:
        R=0
    return R
