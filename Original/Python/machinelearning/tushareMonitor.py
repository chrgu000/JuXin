...
while 1:
    time.sleep(1)
    priceCurrent=float(ts.get_realtime_quotes(stock)['price'])
    timeStr=time.strftime('%H:%M:%S', time.localtime())
    if targetPrice>0:
        if priceCurrent>targetPrice:
            tmp=timeStr+' stock '+stock+' 已大于目标点位。'
            print(tmp)
            tmp=win32api.MessageBox(0,tmp,'Alert!',1)
            if tmp==1:
                print('Monitor continues!')
            ...
            print(timeStr+ 'stock '+stock+' 还差'+str(round(targetPrice-priceCurrent,3))+'个点，才能到达目标点位。')
    else:
        if priceCurrent<-targetPrice:
            tmp = timeStr + ' stock ' + stock + ' 已小于目标点位。'
            print(tmp)
            tmp = win32api.MessageBox(0, tmp, 'Alert!', 1)
            if tmp == 1:
                print('Monitor continues!')
            ...
            print(timeStr+' stock '+stock+' 还差'+str(round(priceCurrent+targetPrice,3))+'个点，才能到达目标点位。')